...
public:
    PthreadMutexAttrGuard(pthread_mutexattr_t & Attr):_Attr(Attr)
    {
        ...
        if (0!=rt)
            _THROW_FMT(SyscallExecuteError, "Failed to execute pthread_mutexattr_init , rt=%d !",rt)
    }
...
PthreadMutex::PthreadMutex()
{
    ...
    if ( 0!=rt )
        _THROW_FMT(SyscallExecuteError, "Failed to execute pthread_mutex_init , rt=%d !",rt)
}
PthreadMutex::~PthreadMutex()
{
...
RawPthreadRWMutex::RawPthreadRWMutex()
{
    ...
    if (0!=rt)
        _THROW_FMT(SyscallExecuteError, "Failed to execute pthread_rwlock_init , rt=%d !",rt)
}
RawPthreadRWMutex::~RawPthreadRWMutex()
{
...
cf_void RawPthreadRWMutex::TryReadLock() cf_const
{
    ...
    {
        if(EBUSY==rt)
            _THROW_FMT(AlreadyLockedInfo, "Failed to execute pthread_rwlock_tryrdlock , rt=%d , already locked!",rt)
        else
            _THROW_FMT(SyscallExecuteError, "Failed to execute pthread_rwlock_tryrdlock , rt=%d !",rt)
    }        
}
...
cf_void RawPthreadRWMutex::TryWriteLock() cf_const
{
    ...
    {
        if(EBUSY==rt)
            _THROW_FMT(AlreadyLockedInfo, "Failed to execute pthread_rwlock_trywrlock , rt=%d , already locked!",rt)
        else
            _THROW_FMT(SyscallExecuteError, "Failed to execute pthread_rwlock_trywrlock , rt=%d !",rt)
    }        
}
