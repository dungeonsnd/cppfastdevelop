...
public:
    PthreadCondAttrGuard(pthread_condattr_t  & Attr):_Attr(Attr)
    {
        ...
        if (0!=rt)
            _THROW_FMT(SyscallExecuteError, "Failed to execute pthread_condattr_init , rt=%d !",rt)
    }
...
PthreadCond::PthreadCond()
{
    ...
    if (0!=rt)
        _THROW_FMT(SyscallExecuteError, "Failed to execute pthread_cond_init , rt=%d !",rt)
}
PthreadCond::~PthreadCond()
{
...
bool PthreadCond::Wait(cf_uint32 timeoutMilliseconds)
{
    ...
    if (0==rt)
        ...
        _THROW_FMT(SyscallExecuteError, "Failed to execute pthread_cond_timedwait , rt=%d !",rt)
}
...
RawPthreadCondition::RawPthreadCondition(pthread_cond_t *cond, pthread_mutex_t *mutex)
    :_cond(cond),
