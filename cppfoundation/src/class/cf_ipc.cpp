...
MemMappedFile::MemMappedFile(cf_const std::string & name, size_t size, cf_int prot, bool lock,bool autoUnmap)
    :_pShm(NULL),
     _size(size),
     _isLocked(lock),
...
cf_void MemMappedFile::Msync(size_t fromIndex, size_t len, cf_int flag)
{
    if(fromIndex+len>size_t(_size))
        _THROW_FMT(ValueError, "fromIndex{%lld}+len{%lld}>size_t(_size) {%lld}!",(cf_llong)fromIndex,(cf_llong)len,(cf_llong)_size)
...
SysVShM::SysVShM(key_t key, size_t size, cf_int shmFlag,bool autoRemove,bool autoDetach)
    :_key(key),
     _shmid(-1),
     _pShmAddr(NULL),
     _autoRemove(autoRemove),
