...
cf_int BinarySearchAscending(T * a, cf_int64 low, cf_int64 high, cf_const T & target, cf_int64 & insertIndex)
{    
#ifdef _ADDCHECK_
    if(low<0)
        return -1;
...
// Use compare class to compare the elements.
template< typename T, typename compare_less>
cf_int BinarySearchDescending(T * a, cf_int64 low, cf_int64 high, cf_const T & target, cf_int64 & insertIndex)
{    
#ifdef _ADDCHECK_
    if(low<0)
        return -1;
