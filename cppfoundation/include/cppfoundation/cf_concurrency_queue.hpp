...
public :
    ...
    ElementType Get()
    {
        ...
        if (_queue.empty() && _stopToPut)
            _THROW(QueueEmptyAndStopToPutException, "This queue is empty and stopped to put!");
...
template <typename ElementType, typename MutexType =PthreadMutex /* or =FakeMutex */>
struct TYPEQueue
{
    typedef RawQueue<std::queue<ElementType>, ElementType, MutexType > Queue;
    typedef RawFastFindQueue<std::queue<ElementType>, ElementType, std::set<ElementType>, MutexType > FastFindQueue;
};
