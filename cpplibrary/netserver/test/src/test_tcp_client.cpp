...
cf_void demo0(size_t size)
{
    ...
    std::shared_ptr < cf::PosixShM > shm( new cf::PosixShM(name.c_str(),size,oflag,prot,mode,autoUnlink) );
    std::shared_ptr < cl::cq::CircularQueue > queue( new cl::cq::CircularQueue(shm->Get(),true,size) );
    Run(queue.get());
}
