...
public:
    ...
    cf_void StopPut() {_pMeta->status =Q_STOP2PUT; }
    cf_void StopPutAndGet() {_pMeta->status =Q_STOP2PUTANDGET; }
    bool IsStopPut() {return Q_STOP2PUT==_pMeta->status; }
    bool IsStopPutAndGet() {return Q_STOP2PUTANDGET==_pMeta->status; }
    
    PQMeta GetPQMeta() { checkPointer(); return _pMeta; }
    
    cf_uint64 & TotalSize() { checkPointer(); return _pMeta->qTotalSize; }
    cf_uint64 & LeftSize() { checkPointer(); return _pMeta->qLeftSize; }
    cf_uint64 & ElementCount() { checkPointer(); return _pMeta->elementCount; }
    
    pthread_mutex_t * Mutex() { checkPointer(); return &(_pMeta->mutex); }
    pthread_cond_t * Cond() { checkPointer(); return &(_pMeta->cond); }
    pid_t & OperationPid() { checkPointer(); return _pMeta->operationPid; }
