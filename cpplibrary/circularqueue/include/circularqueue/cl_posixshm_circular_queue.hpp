...
public:
    ...
    cf_void Put(cf_cpvoid pData,cf_uint64 size) { _queue->Put(pData,size); }
    cf_void TryPut(cf_cpvoid pData,cf_uint64 size) { _queue->TryPut(pData,size); }
    cf_void TimedPut(cf_cpvoid pData,cf_uint64 size,cf_int32 timeoutMilliseconds) { _queue->TimedPut(pData,size,timeoutMilliseconds); }
    
    cf_void Get(std::string & data) { _queue->Get(data); }
